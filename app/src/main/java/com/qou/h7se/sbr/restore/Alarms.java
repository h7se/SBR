        ...
        ContentProviderHandlerEx.XDImport.XDItem.getItemsMatchingPredicate(meta, new DataFilterCallback<ContentProviderHandlerEx.XDImport.XDItem>() {
            ...
            public void run(List<Integer> positions, List<ContentProviderHandlerEx.XDImport.XDItem> result) {
                ...
                for (ContentProviderHandlerEx.XDImport.XDItem item : result) {
                    Pair<String, String[]> selection = HelperEx.buildSelection(item, "name", "createtime");

                    if (!(HelperEx.exists(resolver, Uris.SAMSUNG_ALARMS, selection.first, selection.second))) {
                        builder = ContentProviderOperation
                                .newInsert(Uris.SAMSUNG_ALARMS);
                    } else {
                        builder = ContentProviderOperation
                                .newUpdate(Uris.SAMSUNG_ALARMS)
                ...
                try {
                    ContentProviderResult[] results = resolver.applyBatch(Uris.SAMSUNG_ALARMS.getAuthority(), ops);
